...
path_stencil_newcastle <- "./data/shared/raw/table-stencil.csv"
path_stencil_octo      <- "./data/shared/raw/table-stencil.csv"

path_newc_out   <- "./manipulation/estimation/newcastle-2/newcastle-mmse-k-2.out"
path_octo_out <- "./manipulation/estimation/octo-2/octo-mmse-k-2.out"
...
ds_long_newc <- ds_wide %>% 
  ...
  dplyr::mutate( 
    ...
    ses_group     = factor(SESGRP, levels=c(0,1),labels=c("low","high")),
...
quick_save <- function(g,name,width=550,height=400,dpi=100){
  ggplot2::ggsave(
    ...
    plot=g,
    device = png,
    path = "./reports/combined-3/graphs-bw/",
    width = width,
    height = height,
    # units = "cm",
    dpi = dpi,
...
# black-and-white version - vertical study
pm <- GGally::ggmatrix(
  list(
    "newc" = plot_trajectories_bw(ds_long_newc,sample_size ="max"),
    "octo" = plot_trajectories_bw(ds_long_octo,sample_size ="max")
  ),
  nrow = 1, ncol = 2,
  ...
  xAxisLabels = c("Newcastle 85+","OCTO-Twin")
  # xAxisLabels = "Mini Mental State Exam"
  # legend = 1#,
  # gg = element_text(size=20)
) + theme(
  # legend.position = "left",
...
pm %>% quick_save("fig-3-trajectories",900,900,400)


class_traj_matrix_bw(ds_long_newc, ds_long_octo) %>% 
  quick_save("fig-3-trajectories-wide",1200,450,600)


# g <- ds_lest_old %>% 
d <- ds_lest %>% 
  dplyr::mutate(
    study_class = paste0(study,"_",class)
  )
g <- d %>%
  # ggplot2::ggplot(aes(x=time,y=mmse, color=study,linetype=class))+
  ggplot2::ggplot(aes(x=time,y=mmse, linetype=class))+
  # ggplot2::ggplot(aes(x=time,y=mmse))+
  geom_line(aes(group=study_class), size=1)+
  geom_point(aes(shape=study), size=5.5, fill="black",alpha=.25)+
  scale_shape_manual(values=c("octo"=21,"newcastle"=25))+ 
  # scale_color_manual(values=colors_classes)+
  # scale_linetype_manual(values=c("solid", "dotted"))+ # Change linetypes
  main_theme +
  labs(
    title = "Figure 2: Average estimated MMSE linear trajectories",
    x     = "Years to death",
    y     = "MMSE"
  )+
  labs(shape="Study",linetype="Latent class")+
...
# ---- publisher ---------------------------------------
path_report_1 <- "./reports/combined-3/combined-3.Rmd"
allReports <- c(
  path_report_1
  # ,path_report_2
  # , ...
