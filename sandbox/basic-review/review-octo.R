...
lapply(dto,names)
names(dto[["octo"]][["modeled"]]) # data recoved from SAVEDATA command
names(dto[["octo"]][["estimates"]]) # parameters of the model solution
names(dto[["octo"]][["info"]]) # fit and performance indices
names(dto[["octo"]][["ds_long"]])
# dto[["newc"]][["raw"]] # original data before estimation
modeled <- dto$octo$modeled
# ---- inspect-data -------------------------------------------------------------
temp <- dto$octo$ds_long
...
ds <- dto$octo$ds_long %>% 
  dplyr::mutate(
    ...
    ses_group     = factor(ses_group, levels=c(1,2,3),labels=c("low","medium","high")),
    edu           = as.integer(edu),
...
class_prob <- function(d,xaxis,group,group_label){
  ...
  g <- ggplot2::ggplot(d, aes_string(x=xaxis,y="prob_class_2", fill = group))+
    geom_point(size=3, alpha = .2, shape=21)+
    facet_grid(class~.) + 
...
#   # xaxis = "age_bl"
#   # predictor = "ses_group"
#   g <- ggplot2::ggplot(d, aes_string(x=xaxis,y="prob_class_2",color=group))+
#     geom_point(size=4, alpha = .9, shape=21, fill=NA)+
#     facet_grid(class~.) + 
#     labs(color = group_label)+
#     main_theme
#   return(g)
# }
# class_prob(ds_bl,"age_bl","female")
...
# ---- publisher ---------------------------------------
path_report_1 <- "./reports/basic-review/review-octo.Rmd"
allReports <- c(
  path_report_1
  # ,path_report_2
  # , ...
